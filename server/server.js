...
const { llmManager } = require('./services/llmProviders.js');
...
app.get('/health', (req, res) => {
  res.json({
    ...
    llmConfigured: llmManager.hasConfiguredProvider()
  });
});
...
app.get('/api/llm/providers', (req, res) => {
  res.json({
    ...
    activeProvider: llmManager.activeProvider,
    activeModel: llmManager.activeModel
  });
});
...
app.post('/api/llm/select', (req, res) => {
  try {
    ...
    const result = llmManager.setActiveProvider(provider, model);
    res.json({
      success: true,
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    if (!llmManager.hasConfiguredProvider()) {
      return res.json({
        message: 'AI features are not available. Please configure at least one LLM provider API key in the .env file. Supported providers: OpenAI, Anthropic, Google, Zhipu, DeepSeek, MiniMax.',
        mapCommands: [],
        data: null
      });
    }
...
Format: {"message": "string here", "mapCommands": [optional commands]}`;

    // Use active LLM provider via llmManager
    let result;
    try {
      console.log(`🤖 Sending request to LLM manager (Active: ${llmManager.activeProvider})`);
      result = await llmManager.chat(systemPrompt, message);
      console.log('🤖 AI Response received successfully');
    } catch (llmError) {
      console.error('LLM Chat Error details:', {
        message: llmError.message,
        stack: llmError.stack,
        provider: llmManager.activeProvider,
        promptLength: systemPrompt.length
      });
      // Let the outer catch handle it with the fallback response
      throw llmError;
    }

    res.json({
      message: result?.message || result?.text || 'I analyzed the data but could not formulate a clear response.',
      mapCommands: result?.mapCommands || [],
      data: context
    });

  } catch (error) {
    console.error('Top-level AI API Error:', error);

    // Return fallback response based on data context
    const fallbackStationData = req.body.stationData || [];
    const totalStations = new Set(fallbackStationData.map(d => d.station_id || d.stationId)).size;
    const totalRecords = fallbackStationData.length;

    res.json({
      message: totalRecords === 0
        ? "I don't have enough data to perform a deep analysis. Please import your CSV files first."
        : `I apologize, but I'm having trouble with my external analysis brain right now. Based on the ${totalRecords} records I see here from ${totalStations} stations, I can still help you navigate the map if you have specific questions about the visible points.`,
      mapCommands: [],
...
// Import CSV data and save to database (or in-memory storage)
app.post('/api/data/import', async (req, res) => {
  try {
    ...
    csvData.forEach(record => {
      if (record.station_id && !uniqueStations.has(record.station_id)) {
        uniqueStations.set(record.station_id, {
          stationId: record.station_id,
          projectId: projectId, // Set projectId
    ...
    if (useInMemory) {
      ...
      await inMemoryDb.deleteRecords(projectId);
      await inMemoryDb.deleteStations(projectId);
      await inMemoryDb.insertStations(Array.from(uniqueStations.values()));
      await inMemoryDb.insertRecords(records);
      console.log(`💾 Saved ${records.length} records to in-memory storage for project ${projectId}`);
    } else {
      // Use PostgreSQL database
      const { eq } = require('drizzle-orm');
      await db.delete(pollutionRecords).where(eq(pollutionRecords.projectId, projectId));
      await db.delete(stations).where(eq(stations.projectId, projectId));
...
// Get all data from database (or in-memory storage)
app.get('/api/data/records', async (req, res) => {
  try {
    ...
    if (useInMemory) {
      ...
      records = await inMemoryDb.getRecordsWithStations(projectId);
    } else {
      // Use PostgreSQL database
      let queryBuilder = db
        .select({
          ...
          project_id: pollutionRecords.projectId,
      ...
      if (projectId) {
        queryBuilder = queryBuilder.where(eq(pollutionRecords.projectId, projectId));
      }

      records = await queryBuilder.orderBy(pollutionRecords.sampleDate);
    }
...
// Seed database with sample data from file
app.post('/api/data/seed', async (req, res) => {
  try {
    const csvPath = path.join(__dirname, '../data/dados_exemplo_poluentes_corrigido.csv');
    ...
    dataLines.forEach(line => {
      // Simple CSV split for the seed data (which we know is comma separated)
      ...
      if (values.length >= 8) {
        ...
        records.push({
          stationId: stationId,
          sampleDate: new Date(values[4]),
          polA: parseFloat(values[5]) || 0,
          polB: parseFloat(values[6]) || 0
        });
      }
    });

    if (useInMemory) {
      await inMemoryDb.deleteRecords();
      await inMemoryDb.deleteStations();
      await inMemoryDb.insertStations(Array.from(uniqueStations.values()));
      await inMemoryDb.insertRecords(records);
    } else {
      await db.delete(pollutionRecords);
      await db.delete(stations);
      if (uniqueStations.size > 0) await db.insert(stations).values(Array.from(uniqueStations.values()));
      if (records.length > 0) await db.insert(pollutionRecords).values(records);
    }

    res.json({
      success: true,
      message: `Seeded ${uniqueStations.size} stations and ${records.length} records from sample file`,
      stationsCount: uniqueStations.size,
      recordsCount: records.length
    });
...
// Clear all data from database (or in-memory storage)
app.delete('/api/data/clear', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'All data cleared from storage'
    });
