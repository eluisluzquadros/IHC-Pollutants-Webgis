...
// Fallback for local environments using dotenv
...
Respond in JSON format: {"message": "your response", "mapCommands": [optional map commands]}`;
    ...
    const result = await llmManager.chat(systemPrompt, message);

    res.json({
      message: result.message || 'I apologize, but I encountered an error processing your request.',
...
app.post('/api/data/import', async (req, res) => {
  try {
    ...
    if (useInMemory) {
      ...
      console.log(`💾 Saved ${records.length} records to in-memory storage`);
    } else {
...
app.get('/api/data/records', async (req, res) => {
  try {
    ...
    if (useInMemory) {
      ...
      records = await inMemoryDb.getRecordsWithStations();
    } else {
      // Use PostgreSQL database
      records = await db
        .select({
