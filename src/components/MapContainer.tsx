...
const MapContainer = ({
  ...
  const getSafePadding = useCallback((): maplibregl.PaddingOptions => {
    ...
    const left = isDesktop ? 460 : 16; // sidebar space on desktop
    const top = 80; // header space
    const right = 16;
    const bottom = 90; // legend/controls space
  ...
  useEffect(() => {
    console.log('MapContainer received stationData:', stationData.length, 'records');
  ...
  useEffect(() => {
    ...
    const tilesURL = (import.meta as any).env?.VITE_BASEMAP_URL || 'https://tile.openstreetmap.org/{z}/{x}/{y}.png';
  ...
  useEffect(() => {
    console.log('Data effect triggered:', { mapLoaded, hasMap: !!map.current, dataLength: stationData.length });
    ...
    if (showStationMarkers && !enableClustering) {
      stationData.forEach((station) => {
        const markerElement = createMarkerElement(station.pol_a, station.pol_b, station.unit);
    ...
    // Create GeoJSON data for heatmap & clustering
    const geojsonData = {
    ...
    console.log('Created GeoJSON with', (geojsonData as any).features.length, 'features');
    ...
    if (map.current.getSource(heatSourceId)) {
      ...
      map.current.addLayer({
        ...
        source: heatSourceId,
        paint: {
          "heatmap-weight": ["*", ["+", ["get", "pol_a"], ["get", "pol_b"]], 0.1],
          "heatmap-intensity": 1,
          "heatmap-color": [
            "interpolate",
            ["linear"],
    ...
    if (map.current.getLayer("stations-heatmap")) {
      ...
      map.current.setPaintProperty("stations-heatmap", "heatmap-radius", heatmapRadius);
      map.current.setPaintProperty("stations-heatmap", "heatmap-opacity", heatmapOpacity);
    }

    // Add or update clustering source and layers
    if (map.current.getSource('stations-cluster-data')) {
      (map.current.getSource('stations-cluster-data') as maplibregl.GeoJSONSource).setData(geojsonData as any);
    } else {
      ...
      map.current.on('click', 'clusters', (e: any) => {
        ...
        (map.current!.getSource('stations-cluster-data') as any).getClusterExpansionZoom(clusterId, (err: any, zoom: number) => {
          if (err) return;
    ...
    ['clusters', 'cluster-count'].forEach((layerId) => {
      if (map.current!.getLayer(layerId)) {
        map.current!.setLayoutProperty(layerId, 'visibility', enableClustering ? 'visible' : 'none');
      }
    });
  ...
  useEffect(() => {
    ...
    (window as any).resetMapView = () => {
      if (map.current && boundsRef.current) {
        map.current.fitBounds(boundsRef.current, { padding: getSafePadding(), maxZoom: 10 });
      }
    };
  ...
  return (
    <Card className="w-full h-full bg-[#2C3E50] overflow-hidden rounded-lg shadow-lg relative">
      ...
      {tooltip.visible && tooltip.station && (
        <StationTooltip
          stationName={tooltip.station.name}
          sampleDate={tooltip.station.sampleDate}
          polA={tooltip.station.polA}
          polB={tooltip.station.polB}
          unit={tooltip.station.unit}
