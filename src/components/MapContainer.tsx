...
// Make sure window.L is set before plugins try to use it
...
interface MapContainerProps {
  ...
  onSpatialFilterUpdate?: (geometry: any | null) => void;
}


// Function to create custom SVG marker with dynamic bar chart
const createStationMarkerSVG = (polA: number, polB: number, unit: string, visiblePollutants: string[] = ["pol_a", "pol_b"]) => {
  const showA = visiblePollutants.includes("pol_a");
  const showB = visiblePollutants.includes("pol_b");
  const activeCount = (showA ? 1 : 0) + (showB ? 1 : 0);

  const maxValue = Math.max(polA, polB, 10); // Ensure minimum scale
  const barHeight = 30;
  const barWidth = 8;
  const spacing = 2;
  const totalWidth = (barWidth * activeCount) + (activeCount > 1 ? spacing : 0) + 4; // 4px padding
  ...
  return `
    <svg width="${totalWidth}" height="${totalHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${barWidth / 2 + 2}" y="${totalHeight - 1}" font-family="Arial" font-size="6" fill="#2C3E50" text-anchor="middle">A</text>
      ` : ''}
      
      <!-- Bar B (pol_b) -->
      ${showB ? `
      <rect x="${showA ? 2 + barWidth + spacing : 2}" y="${totalHeight - barBHeight - 5}" 
            width="${barWidth}" height="${barBHeight}" 
            fill="${colorB}" stroke="#2C3E50" stroke-width="0.5"/>
      <text x="${showA ? 6 + barWidth + spacing : barWidth / 2 + 2}" y="${totalHeight - 1}" font-family="Arial" font-size="6" fill="#2C3E50" text-anchor="middle">B</text>
      ` : ''}
    </svg>
  `;
};
...
const createStationIcon = (polA: number, polB: number, unit: string, visiblePollutants?: string[]) => {
  return L.divIcon({
    html: createStationMarkerSVG(polA, polB, unit, visiblePollutants),
...
const createFocusedStationIcon = (polA: number, polB: number, unit: string, visiblePollutants?: string[]) => {
  return L.divIcon({
    html: `<div style="position: relative; animation: pulse 2s infinite;">
             ${createStationMarkerSVG(polA, polB, unit, visiblePollutants)}
...
const createHighlightedStationIcon = (polA: number, polB: number, unit: string, visiblePollutants?: string[]) => {
  return L.divIcon({
...
// Function to create custom popup content for stations
const createStationPopupContent = (station: any, visiblePollutants: string[]) => {
  const showA = visiblePollutants.includes("pol_a");
  const showB = visiblePollutants.includes("pol_b");

  return `
    <div style="color: #2C3E50; font-family: Arial, sans-serif; min-width: 250px;">
      <h3 style="margin: 0 0 8px 0; color: #3B82F6; font-size: 16px;">${station.station_name || 'Station ' + station.station_id}</h3>
      <p style="margin: 2px 0; font-size: 11px; color: #666;">Station ID: ${station.station_id}</p>
      <p style="margin: 8px 0 4px 0; font-weight: bold; font-size: 12px; border-bottom: 1px solid #eee; padding-bottom: 2px;">Pollution Levels:</p>
      <div style="display: flex; gap: 15px; margin: 10px 0;">
        ...
        <div style="text-align: center; flex: 1;">
          <div style="width: 20px; height: ${Math.max((station.pol_a / Math.max(station.pol_a, station.pol_b, 10)) * 30, 2)}px; 
                      ...
                      margin: 0 auto 4px; border: 1px solid #ccc; border-radius: 2px;"></div>
          <small style="display: block; font-size: 10px;"><strong>Pol A</strong></small>
          <small style="display: block; font-size: 10px;">${station.pol_a.toFixed(2)} ${station.unit}</small>
        </div>
        ` : ""}
        ${showB ? `
        <div style="text-align: center; flex: 1;">
          <div style="width: 20px; height: ${Math.max((station.pol_b / Math.max(station.pol_a, station.pol_b, 10)) * 30, 2)}px; 
                      ...
                      margin: 0 auto 4px; border: 1px solid #ccc; border-radius: 2px;"></div>
          <small style="display: block; font-size: 10px;"><strong>Pol B</strong></small>
          <small style="display: block; font-size: 10px;">${station.pol_b.toFixed(2)} ${station.unit}</small>
        </div>
        ` : ""}
      </div>
      <div style="margin-top: 8px; font-size: 10px; color: #888; display: flex; justify-content: space-between;">
        ...
        <span>📅 ${station.sample_dt || 'No date'}</span>
      </div>
    </div>
  `;
...
const MapContainer = ({
  ...
  onSpatialFilterUpdate,
}: MapContainerProps) => {
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<L.Map | null>(null);
  const [mapLoaded, setMapLoaded] = useState(false);
  const [mapError, setMapError] = useState<string | null>(null);
  // Track previous dataset length to detect a new data load vs. settings-only change
  ...
  // Keep callback fresh for Leaflet event listeners
  const filterCallbackRef = useRef(onSpatialFilterUpdate);
  useEffect(() => {
    filterCallbackRef.current = onSpatialFilterUpdate;
  }, [onSpatialFilterUpdate]);

  // Handle focused station (auto-pan) — reads zoom directly from map, no state needed
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      // Create the map — disable default zoom control (topleft) and add at topright
      ...
      // Apply a patch for React/Leaflet-Draw where Rectangle closes prematurely and creates a tiny shape
      if ((L as any).Draw && (L as any).Draw.Rectangle && !(L as any).Draw.Rectangle.prototype._patchedMouseUp) {
        const OriginalMouseUp = (L as any).Draw.Rectangle.prototype._onMouseUp;
        (L as any).Draw.Rectangle.prototype._onMouseUp = function(e: any) {
          if (this._shape && this._map) {
            ...
            // Ignore mouse up if the box drawn is smaller than 15x15 pixels on the screen.
            // This prevents a single click jitter from instantly finishing a 0-size rectangle!
        ...
        (L as any).Draw.Rectangle.prototype._patchedMouseUp = true;
      }
      ...
      map.current.on((L as any).Draw.Event.CREATED, (e: any) => {
        // Enforce single shape: clear previous
        ...
        if (e.layerType === 'circle') {
          geojson.properties = { ...geojson.properties, radius: layer.getRadius() };
        ...
        if (filterCallbackRef.current) filterCallbackRef.current(geojson);
      });

      map.current.on((L as any).Draw.Event.EDITED, (e: any) => {
      ...
      map.current.on((L as any).Draw.Event.DELETED, () => {
        if (filterCallbackRef.current) filterCallbackRef.current(null);
      });
      // -----------------------------------
  ...
  useEffect(() => {
    ...
    if (showHeatmap && heatmapOpacity > 0) {
      ...
      const getPollutionValue = (s: any) => {
        if (showA && showB) return (s.pol_a + s.pol_b) / 2;
        if (showA) return s.pol_a;
        if (showB) return s.pol_b;
        return 0;
      };

      const pollutionValues = validStations.map(getPollutionValue);
      ...
      const heatmapData = validStations.map(station => {
        const val = getPollutionValue(station);
        const intensity = Math.min(val / maxPollution, 1);
      ...
      try {
        // Create heatmap layer — solid gradient colors (leaflet.heat ignores rgba alpha),
        // overall opacity controlled via canvas.style after layer creation
        heatmapLayerRef.current = L.heatLayer(heatmapData as [number, number, number][], {
          radius: heatmapRadius,
          blur: 20,
          maxZoom: 17,
          max: 1.0,
          minOpacity: Math.max(0.5, heatmapOpacity * 0.6),
          gradient: {
            0.0: '#00e600',   // Verde
            0.3: '#80ff00',   // Verde-amarelo
            0.5: '#ffff00',   // Amarelo
            0.7: '#ff8000',   // Laranja
            0.85: '#ff3300',  // Vermelho-laranja
            1.0: '#cc0000'    // Vermelho
          }
        });
        ...
        // Apply overall canvas opacity — this is how leaflet.heat opacity actually works
        if (heatmapLayerRef.current._canvas) {
          heatmapLayerRef.current._canvas.style.opacity = String(Math.max(heatmapOpacity, 0.7));
        }

        console.log(`✓ Heatmap created with ${validStations.length} data points, radius: ${heatmapRadius}, opacity: ${heatmapOpacity}`);
      } catch (error) {
    ...
    // 2. CREATE STATION MARKERS (HANDLED IN CLUSTERING SECTION BELOW)
    // Removed redundant loop to prevent marker layering issues
            ...
            return `<div style="
                     ...
                     ">
                       ...
                       <div style="color: #6B7280; margin-top: 2px;">
                         ${visiblePollutants.includes("pol_a") ? `Pol A: <strong>${reading.pol_a}</strong>` : ""}
                         ${visiblePollutants.includes("pol_a") && visiblePollutants.includes("pol_b") ? " | " : ""}
                         ${visiblePollutants.includes("pol_b") ? `Pol B: <strong>${reading.pol_b}</strong>` : ""}
                         ${reading.unit}
                       </div>
                     </div>`;
          }).join('');
    ...
    if (showStationMarkers || showAdvancedMarkers || enableStationClustering || enableRecordClustering) {

      // Helper: pick icon based on visualization mode
      const getMarkerIcon = (station: any, isFocused: boolean, isHighlighted: boolean) => {
        if (showAdvancedMarkers) {
          if (isFocused) return createFocusedStationIcon(station.pol_a, station.pol_b, station.unit, visiblePollutants);
          if (isHighlighted) return createHighlightedStationIcon(station.pol_a, station.pol_b, station.unit, visiblePollutants);
          return createStationIcon(station.pol_a, station.pol_b, station.unit, visiblePollutants);
        }
        // Default: POI pin style
        if (isFocused) return L.divIcon({
          html: `<div style="position:relative;animation:pulse 2s infinite;">
                   <svg width="25" height="41" viewBox="0 0 25 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <path d="M12.5 0C5.6 0 0 5.6 0 12.5C0 21.9 12.5 41 12.5 41S25 21.9 25 12.5C25 5.6 19.4 0 12.5 0Z" fill="#3B82F6"/>
                     <circle cx="12.5" cy="12.5" r="4" fill="white"/>
                   </svg>
                   <div style="position:absolute;top:-5px;left:-5px;width:35px;height:35px;border:3px solid #3B82F6;border-radius:50%;box-shadow:0 0 20px rgba(59,130,246,0.6);"></div>
                 </div>`,
          className: 'custom-div-icon focused-station',
          iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34],
        });
        if (isHighlighted) return L.divIcon({
          html: `<div style="position:relative;">
                   <svg width="25" height="41" viewBox="0 0 25 41" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          className: 'custom-div-icon highlighted-station',
          iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34],
        });
        return DefaultIcon;
      };
      ...
      if (enableStationClustering) {
        ...
        stationArray.forEach(station => {
          const marker = L.marker([station.lat, station.lon], {
            icon: getMarkerIcon(station, false, false)
          });

          marker.bindPopup(createStationPopupContent(station, visiblePollutants));
        ...
        stationGroups.forEach(stationGroup => {
          ...
          const marker = L.marker([representative.lat, representative.lon], {
            icon: getMarkerIcon(representative, false, false)
          });
          ...
          const recordsHtml = stationGroup.map((record, idx) => `
            <div style="padding: 4px; margin: 2px 0; background: ${idx % 2 === 0 ? '#f8f9fa' : '#ffffff'}; border-radius: 3px;">
              <small><strong>Reading ${idx + 1}:</strong> 
                ${visiblePollutants.includes("pol_a") ? `Pol A: ${record.pol_a.toFixed(2)}` : ""}
                ${visiblePollutants.includes("pol_a") && visiblePollutants.includes("pol_b") ? ", " : ""}
                ${visiblePollutants.includes("pol_b") ? `Pol B: ${record.pol_b.toFixed(2)}` : ""}
                ${record.unit}
              </small>
      ...
      } else if (showStationMarkers || showAdvancedMarkers) {
        // Individual markers without clustering
        console.log(`Creating ${validStations.length} individual station markers (advanced: ${showAdvancedMarkers})`);
        ...
        validStations.forEach(station => {
          ...
          const icon = getMarkerIcon(station, isFocused, isHighlighted);
          ...
          if (onStationHover && onStationLeave) {
            ...
            marker.on('mouseout', () => onStationLeave());
          }
    ...
    // 4. FIT MAP ONLY WHEN A NEW DATASET IS LOADED
    // Detect new data load by comparing lengths — settings changes won't trigger refitting
    const isNewDataLoad = stationData.length !== prevDataLengthRef.current;
    prevDataLengthRef.current = stationData.length;

    if (isNewDataLoad && validStations.length > 0 && markersRef.current.length > 0) {
  ...
  }, [mapLoaded, stationData, showStationMarkers, showAdvancedMarkers, showHeatmap, heatmapOpacity, heatmapRadius, enableStationClustering, enableRecordClustering, showRecordCount, clearMapLayers, visiblePollutants]);
  ...
  return (
    <Card className="w-full h-full bg-[#2C3E50] overflow-hidden rounded-lg shadow-lg relative">
      ...
      <div className="absolute bottom-6 left-6 z-[1000] pointer-events-none flex flex-col gap-2">
        <div className="bg-background/95 backdrop-blur-md p-3 sm:p-4 rounded-xl border border-border/50 shadow-2xl pointer-events-auto max-w-[250px]">
          <h4 className="text-[10px] font-bold uppercase tracking-widest text-primary mb-3">Legenda do Mapa</h4>
          <div className="flex flex-col gap-2 text-[10px] font-medium text-foreground/80">
            <div className="flex items-center gap-2.5">
              <div className="w-2.5 h-2.5 rounded-full bg-[#2ECC71] shadow-[0_0_8px_rgba(46,204,113,0.5)]"></div>
              <span>Boa Qualidade (0 - 3)</span>
            </div>
            <div className="flex items-center gap-2.5">
              <div className="w-2.5 h-2.5 rounded-full bg-[#F39C12] shadow-[0_0_8px_rgba(243,156,18,0.5)]"></div>
              <span>Moderada (3 - 7)</span>
            </div>
            <div className="flex items-center gap-2.5">
              <div className="w-2.5 h-2.5 rounded-full bg-[#E74C3C] shadow-[0_0_8px_rgba(231,76,60,0.5)]"></div>
              <span>Ruim (&gt; 7)</span>
            </div>
          </div>
          <div className="mt-3 pt-3 border-t border-border/50 text-[9px] text-foreground/50 leading-tight">
            Use as ferramentas de desenho no topo à esquerda para filtrar os dados por área no mapa.
          </div>
        </div>
      </div>
