          ...
          <div className="kpi-card bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200">
            <div className="kpi-card-content">
              <div className="kpi-card-info">
                ...
                <div className={`text-xs px-2 py-1 rounded-full ${polAStatus.status === 'Low' ? 'text-emerald-700 bg-emerald-100' :
          ...
          <div className="kpi-card bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
            <div className="kpi-card-content">
              <div className="kpi-card-info">
                ...
                <div className={`text-xs px-2 py-1 rounded-full ${polBStatus.status === 'Low' ? 'text-emerald-700 bg-emerald-100' :
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          ...
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics.chartData} margin={{ top: 10, right: 30, left: 10, bottom: 10 }}>
                ...
                <YAxis
                  ...
                  axisLine={false}
                  tickLine={false}
                  label={analytics.useDualAxis ? { value: 'Pol A', angle: -90, position: 'insideLeft', fontSize: 10 } : null}
                />
                {analytics.useDualAxis && (
                  <YAxis
                    yAxisId="right"
                    orientation="right"
                    tick={{ fontSize: 10 }}
                    axisLine={false}
                    tickLine={false}
                    label={{ value: 'Pol B', angle: 90, position: 'insideRight', fontSize: 10 }}
                  />
                )}
                <Tooltip
                  contentStyle={{
                ...
                />
                {visiblePollutants.includes("pol_a") && (
                  <Bar yAxisId="left" dataKey="pol_a" fill="#3b82f6" radius={[2, 2, 0, 0]} />
                )}
                {visiblePollutants.includes("pol_b") && (
                  <Bar yAxisId={analytics.useDualAxis ? "right" : "left"} dataKey="pol_b" fill="#8b5cf6" radius={[2, 2, 0, 0]} />
                )}
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          ...
          <div className="h-40 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={analytics.chartData.slice(0, 10)} margin={{ top: 10, right: 30, left: 10, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis
                  dataKey="name"
                  tick={{ fontSize: 10 }}
                ...
                {visiblePollutants.includes("pol_b") && (
                  <Line
                    yAxisId={analytics.useDualAxis ? "right" : "left"}
                    type="monotone"
                    dataKey="pol_b"
                    stroke="#8b5cf6"
                    strokeWidth={2}
