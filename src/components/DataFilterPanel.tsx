...
export type StationFilters = {
  ...
  polAMin?: number;
  polBMin?: number;
  visiblePollutants: string[]; // ['pol_a', 'pol_b']
};
...
export default function DataFilterPanel({
  ...
  const [visiblePollutants, setVisiblePollutants] = useState<string[]>(value?.visiblePollutants ?? ["pol_a", "pol_b"]);
  ...
  useEffect(() => {
    ...
    setVisiblePollutants(value.visiblePollutants ?? ["pol_a", "pol_b"]);
  }, [value?.stationQuery, value?.dateFrom, value?.dateTo, value?.polAMin, value?.polBMin, value?.visiblePollutants]);
  ...
  const currentFilters: StationFilters = useMemo(
    ...
    [stationQuery, dateFrom, dateTo, polAMin, polBMin, visiblePollutants]
  );
  ...
  const resetFilters = () => {
    ...
    setPolAMin(0);
    setPolBMin(0);
    setVisiblePollutants(["pol_a", "pol_b"]);
    onChange?.(cleared);
    ...
    <Card className={`bg-white dark:bg-[hsl(var(--card))] text-sm w-full max-w-md ${className}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3 pt-2 border-t border-gray-100">
          <Label className="text-xs font-semibold uppercase tracking-wider text-gray-500">Visible Pollutants</Label>
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center space-x-2">
              <Checkbox
                id="show-pola"
                checked={visiblePollutants.includes("pol_a")}
                onCheckedChange={(checked) => {
                  if (checked) {
                    setVisiblePollutants(prev => [...prev, "pol_a"]);
                  } else {
                    setVisiblePollutants(prev => prev.filter(p => p !== "pol_a"));
                  }
                }}
              />
              <Label htmlFor="show-pola" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                Pollution A
              </Label>
            </div>
            <div className="flex items-center space-x-2">
              <Checkbox
                id="show-polb"
                checked={visiblePollutants.includes("pol_b")}
                onCheckedChange={(checked) => {
                  if (checked) {
                    setVisiblePollutants(prev => [...prev, "pol_b"]);
                  } else {
                    setVisiblePollutants(prev => prev.filter(p => p !== "pol_b"));
                  }
                }}
              />
              <Label htmlFor="show-polb" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                Pollution B
              </Label>
            </div>
          </div>
          {visiblePollutants.length === 0 && (
            <p className="text-[10px] text-red-500 font-medium">Please select at least one pollutant</p>
          )}
        </div>
